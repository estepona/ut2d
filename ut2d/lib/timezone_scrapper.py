...
class TimezoneScrapper:
  ...
  Some popular search engines like Google or Bing supports getting the time and timezone info of a given place with
  one simple query "xx time".

  Pre-selected search engines will be tested for availability, and then scrapped to collect the timezone information.
  """
  def __init__(self):
    self.search_engines = []
    self.timezone = None
  ...
  def _scrap_bing(self, city: str) -> Optional[str]:
    ...
    try:
      ...
      tz_text = soup.select_one(
        'div[class="baselClock"] div[class="b_focusLabel"]'
      ).text
