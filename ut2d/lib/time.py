...
class Time:
  ...
  def is_valid(ut: Union[int, float]) -> bool:
    """Check if the input unix timestamp is either 10-digit (in seconds) or 13-digit (in milliseconds).
  ...
  def in_city(self, city: str) -> datetime:
    """Get the datetime in a given city by first scrap the timezone info with a scrapper, then call the
    in_timezone method.
    """
    ts = TimezoneScrapper()
